...
public class HalamanTambahHadiah extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnsubmit = (Button) findViewById(R.id.btnTambahHadiah);
    ...
    public void btnSubmitOrtuTambahHadiah (View v) {
        if (foto != null) {
...
//                            startActivity(new Intent(getApplicationContext(), JadwalFragment.class));
                            Intent intent = new Intent(HalamanTambahHadiah.this, HalamanPenjadwalanMandiri.class);
                            startActivity(intent);

                            TambahHadiahOrtuModel tambahHadiahOrtuModel = new TambahHadiahOrtuModel(etnamahadiah.getText().toString(),
                                    etpoinhadiah.getText().toString(), etdeskripsihadiah.getText().toString(),
                                    taskSnapshot.getDownloadUrl().toString());

                            //save image info in firebase database
                            String uploadId = mDatabase.push().getKey();
                            mDatabase.child(uploadId).setValue(tambahHadiahOrtuModel);
                        }
                    })
    ...
    private void photoBuilder() {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(HalamanTambahHadiah.this);
        builder.setTitle("Lampirkan Foto");
        builder.setItems(options, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int item) {
                boolean result = Utility.checkPermission(HalamanTambahHadiah.this);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == RESULT_OK) {
            ...
            Toast.makeText(HalamanTambahHadiah.this, "Foto gagal diambil, silahkan coba lagi", Toast.LENGTH_LONG).show();
        }
    }

    private void onCaptureImageResult(Intent data) {
        foto = Uri.fromFile(files);
        Picasso.with(HalamanTambahHadiah.this).load(files).resize(imfoto.getWidth(), 500).centerCrop().into(imfoto);
    }
    ...
    private void onSelectFromGalleryResult(Intent data) {
        ...
        if (data != null) {
            ...
            Cursor cursor = HalamanTambahHadiah.this.getContentResolver().query(selectedImage, filePathColumn, null, null, null);
            cursor.moveToFirst();
            ...
            Picasso.with(HalamanTambahHadiah.this).load(file).resize(imfoto.getWidth(), 500).centerCrop().into(imfoto);
        }
    }
